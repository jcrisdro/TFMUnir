...
class OutputService:
    ...
    def show_video(self, list_videos: list = None, folder_name: str = None):
        out = cv2.VideoWriter(f"{ROOT_PROJECT}/uploads/{folder_name}/output.avi", cv2.VideoWriter_fourcc(*'DIVX'), 
                              20.0, (640, 480))
        ...
        for video in list_videos:
            ...
            while _.isOpened():
                ...
                if ret:
                    ...
                    cv2.putText(frame, f"Input: {video['INPUT_TEXT']}", self.position_top, self.font, self.font_scale, 
                                self.font_color, self.line_type)
                    cv2.putText(frame, f"Output: {video['SENTENCE']}", self.position_bottom, self.font, self.font_scale, 
                                self.font_color, self.line_type)
                    out.write(frame)
